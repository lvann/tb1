...
\jri{need to add explanation of accession names in separate table or in figure legend.} \mbh{I'd say add this to the legend}
...
\res{We agree that a planned grow-out with previously determined genotypes could be useful, but our interests lie in the effects of the \emph{Hopscotch} in the genetic background of individuals sampled from a naturally-occurring population.
It has previously been shown that \emph{Hopscotch} has a phenotypic effect in a controlled isogenic background, \mbh{CITE} and our pilot studies with a small sample of teosinte inbred lines also revealed a minor effect.
As leaf tissue can only be sampled from reasonably large seedligns and genotyping takes some time, we initially decided against trying to grow up many extra seedlings and genotype.  This is, however, the approach we used for our second-growout to even sample sizes. \jri{true?} Note that because plants were situated in the greenhouse before genotyping, genotypes were effectively randomize with respect to position in the greenhouse.}
