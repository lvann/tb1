...
We would like to thank you and two anonymous reviewers for your helpful comments and suggestions regarding our manuscript. We have addressed the majority of the issues raised in a revised version of our manuscript, but, while we feel that suggested new experiments would provide interesting results, they would require considerable extra time (many months) and are outside of the scope of this current work.  We have attached a revised version of our manuscript, including the suggested changes, and have detailed our responses in blue below. We hope that you are willing to consider our revised manuscript as a resubmission.\\\
...
\res{Our intention with the NJ trees was to show that there is no clear signal of the Hopscotch genotype at neighboring sequence.  If the Hopscotch element were under selection in toesinte populations we might expect to see such a signal due to Hill-Robertson effects -- in fact the original discovery of Hopscotch was made using sequence that delineated sequences with and without the insertion but was almost 60Kb away from the Hopscotch itself.  We agree with the reviewer that the trees are a bit confusing and difficult to interpret, and the large polytomies don't provide much information (beyond the lack of association with Hopscotch genotype).  We have opted to keep the NJ trees in the supplement, where interested readers can view them but they don't unnecessarily complicate the main story.  We have added explanation of the accession names as well.}

\jri{need to add explanation of accession names in separate table or in figure legend.}
...
\res{We agree that a planned grow-out with previously determined genotypes would be useful, but our samples were collected as seed from a naturally occuring population.  We know the Hopscotch has a phenotypic effect in a controlled isogenic background (and a preliminary experiment with a small sample of teosinte inbred lines also saw an effect), but we were interested in knowing whether the Hopsctoch had an effect in the segregating genetic backgrounds present in a natural population.  This prevents us from using crosses.  As leaf tissue can only be sampled from reasonably large seedligns and genotyping takes some time, we initially decided against trying to grow up many extra seedlings and genotype.  This is, however, the approach we used for our second-growout to even sample sizes. \jri{true?} Note that because plants were situated in the greenhouse before genotyping, genotypes were effectively randomize with respect to position in the greenhouse.}
...
\res{We have fixed the eror in reporting sample size, and have added sample size information for all three genotypes.} \jri{I didn't see sample sizes for each of the genotpyes. are they there? if not, please add.}
